...
static int
set_window(sensor_t *sensor, ov2640_sensor_mode_t mode, int offset_x, int offset_y, int max_x, int max_y, int w,
           int h) {
    int ret = 0;
    ...
    uint8_t win_regs[][2] = {
            ...
            {XOFFL,    offset_x & 0xFF},
            {YOFFL,    offset_y & 0xFF},
            {VHYX,     ((max_y >> 1) & 0X80) | ((offset_y >> 4) & 0X70) | ((max_x >> 5) & 0X08) |
                       ((offset_y >> 8) & 0X07)},
            {TEST,     (max_x >> 2) & 0X80},
            {ZMOW,     (w) & 0xFF},
            {ZMOH,     (h) & 0xFF},
