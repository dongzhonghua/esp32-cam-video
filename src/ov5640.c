...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div, int pre_div, bool root_2x,
    ...
    unsigned int PLL_CLK = pll_bypass ? (xclk) : (VCO / pll_sys_div * 2 /
                                                  5);//5 here is 10bit mode / 2, for 8bit it should be 4 (reg 0x3034)
...
static int set_pll(sensor_t *sensor, bool bypass, uint8_t multiplier, uint8_t sys_div, uint8_t pre_div, bool root_2x,
    ...
    ESP_LOGI(TAG,
             "Set PLL: bypass: %u, multiplier: %u, sys_div: %u, pre_div: %u, root_2x: %u, pclk_root_div: %u, pclk_manual: %u, pclk_div: %u",
...
static int set_special_effect(sensor_t *sensor, int effect) {
    ...
    uint8_t * regs = (uint8_t * )
    sensor_special_effects[effect];
...
static int set_saturation(sensor_t *sensor, int level) {
    ...
    uint8_t * regs = (uint8_t * )
    sensor_saturation_levels[level + 4];
    for (int i = 0; i < 11; i++) {
...
static int
set_res_raw(sensor_t *sensor, int startX, int startY, int endX, int endY, int offsetX, int offsetY, int totalX,
