...
void IRAM_ATTR
ll_cam_send_event(cam_obj_t
*cam,
cam_event_t cam_event, BaseType_t
* HPTaskAwoken)
{
if (
xQueueSendFromISR(cam
->event_queue, (void *)&cam_event, HPTaskAwoken) != pdTRUE) {
ll_cam_stop(cam);
cam->
                ...
                if (cam_event == CAM_IN_SUC_EOF_EVENT) {
                    if (!cam_obj->psram_mode) {
                        ...
                        frame_buffer_event->len += ll_cam_memcpy(cam_obj,
                                                                 ...
                                                                 &cam_obj->dma_buffer[
                                                                         (cnt % cam_obj->dma_half_buffer_cnt) *
                    ...
                    if (cnt || !cam_obj->jpeg_mode || cam_obj->psram_mode) {
                        if (cam_obj->jpeg_mode) {
                            if (!cam_obj->psram_mode) {
                                if (cam_obj->fb_size < (frame_buffer_event->len + pixels_per_dma)) {
                                    ...
                                    frame_buffer_event->len += ll_cam_memcpy(cam_obj,
                                                                             &frame_buffer_event->buf[frame_buffer_event->len],
                                                                             &cam_obj->dma_buffer[(cnt %
                                                                                                   cam_obj->dma_half_buffer_cnt) *
                                                                                                  cam_obj->dma_half_buffer_size],
                                                                             cam_obj->dma_half_buffer_size);
                                }
                            }
                        ...
                        if (!cam_obj->frames[frame_pos].en &&
                            ...
                            if (xQueueReceive(cam_obj->frame_buffer_queue, &fb2, 0) == pdTRUE) {
                                ...
                                if (xQueueSend(cam_obj->frame_buffer_queue, (void *) &frame_buffer_event, 0) !=
                                    pdTRUE) {
                                    cam_obj->frames[frame_pos].en = 1;
...
static esp_err_t cam_dma_config() {
    ...
    for (int x = 0; x < cam_obj->frame_cnt; x++) {
        ...
        cam_obj->frames[x].fb.buf = (uint8_t *) heap_caps_malloc(fb_size * sizeof(uint8_t) + dma_align,
                                                                 MALLOC_CAP_SPIRAM);
        ...
        if (cam_obj->psram_mode) {
            ...
            cam_obj->frames[x].dma = allocate_dma_descriptors(cam_obj->dma_node_cnt, cam_obj->dma_node_buffer_size,
    ...
    if (!cam_obj->psram_mode) {
        ...
        cam_obj->dma = allocate_dma_descriptors(cam_obj->dma_node_cnt, cam_obj->dma_node_buffer_size,
                                                cam_obj->dma_buffer);
