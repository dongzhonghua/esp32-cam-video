...
static size_t IRAM_ATTR
ll_cam_dma_filter_jpeg(uint8_t
* dst,
const uint8_t *src, size_t
len)
{
...
static size_t IRAM_ATTR
ll_cam_dma_filter_grayscale(uint8_t
* dst,
const uint8_t *src, size_t
len)
{
...
static size_t IRAM_ATTR
ll_cam_dma_filter_grayscale_highspeed(uint8_t
* dst,
const uint8_t *src, size_t
len)
{
...
static size_t IRAM_ATTR
ll_cam_dma_filter_yuyv(uint8_t
* dst,
const uint8_t *src, size_t
len)
{
...
static size_t IRAM_ATTR
ll_cam_dma_filter_yuyv_highspeed(uint8_t
* dst,
const uint8_t *src, size_t
len)
{
...
static bool ll_cam_calc_rgb_dma(cam_obj_t *cam) {
    ...
    ESP_LOGI(TAG,
             "node_size: %4u, nodes_per_line: %u, lines_per_node: %u, dma_half_buffer_min: %5u, dma_half_buffer: %5u, lines_per_half_buffer: %2u, dma_buffer_size: %5u, image_size: %u",
...
esp_err_t ll_cam_set_sample_mode(cam_obj_t *cam, pixformat_t pix_format, uint32_t xclk_freq_hz, uint8_t sensor_pid) {
    if (pix_format == PIXFORMAT_GRAYSCALE) {
        ...
        if (sensor_pid != OV2640_PID && sensor_pid != OV3660_PID && sensor_pid != OV5640_PID &&
